...
const DemoNameStep: React.FC<StepComponentProps<DemoNamePayload>> = ({
    ...
    const [fieldValues, setFieldValues] = React.useState<Record<string, string>>(() =>
        fields?.reduce(
            (acc, field) => ({
                ...acc,
                [field.key]: initialData?.[field.key] ?? coreContext?.flowData?.[field.key] ?? '',
            }),
            {}
        )
    )
    ...
    return (
        <div className="space-y-8 p-4 animate-fade-left">
            <p>This is a simple example on how you can capture user input in your onboarding flow</p>
            {fields?.map((field) => (
                <div key={field.key} className="space-y-1">
                    ...
                    <Input
                        ...
                        className={`w-full ${fieldErrors[field.key] ? 'border-red-500 focus:border-red-500' : ''}`}
