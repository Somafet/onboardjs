...
const OnboardingUIManager: React.FC<OnboardingUIManagerProps> = ({
    ...
    const { state, currentStep, isLoading, next, skip, reset, previous, renderStep } =
        useOnboarding<AppOnboardingContext>()
    const [currentActiveStepData, setCurrentActiveStepData] = useState<Record<string, unknown>>({})
    ...
    if (state.error) return <ErrorScreen error={state.error} onRetry={() => reset({ steps: stepsConfig })} />
                ...
                {CompletedScreen ?? (
                    <Card className="w-full max-w-lg mx-auto my-8 text-center">
                        <CardHeader>
                            ...
                            <CardTitle className="text-3xl text-green-600">Demo Completed!</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-gray-700 text-sm">
                                ...
                                <br />
                                <br />
                                You can customize the steps, components, and flow to fit your application’s needs. Check
                        ...
                        <CardFooter className="justify-center mt-8 space-x-4">
                            ...
                            <Button onClick={() => router.push('https://onboardjs.com')}>Go to Homepage</Button>
                        </CardFooter>
                    </Card>
                )}
            </>
    ...
    if (!currentStep) {
        return <div className="p-10 text-center text-gray-500">No active onboarding step.</div>
    }
        ...
        <div className="w-full mx-auto h-full flex flex-col">
            <CardHeader className="pb-4">
                <div className="flex justify-between items-start mb-2">
                    <div>
                        <CardTitle className="text-2xl font-bold mb-2">{currentStep.payload?.title}</CardTitle>
            ...
            <CardContent className="min-h-[250px] p-6 sm:pt-12">{renderStep()}</CardContent>
                ...
                <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex w-full sm:w-auto space-x-3 justify-end">
                        ...
                        {(state.canGoNext || state.isLastStep) && (
                            <Button onClick={() => next(currentActiveStepData)} className="animate-fade">
