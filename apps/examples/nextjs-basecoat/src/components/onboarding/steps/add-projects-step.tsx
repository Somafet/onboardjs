...
const Aside = () => (
    <div>
        <h3 className="text-lg font-semibold">Identify your key project to track in Increaser</h3>
        <p className="text-muted mt-6">
            Begin by adding projects that represent your focused endeavors. These projects will help you organize your
            tasks and goals effectively.
        </p>
...
export default function AddProjectsStep({ coreContext }: StepComponentProps) {
    ...
    const addProject = () => {
        ...
        if (projectName) {
            updateContext({
                flowData: {
                    projects: [...(coreContext.flowData.projects ?? []), { name: projectName, tasks: [] }],
                },
            })
    ...
    const removeProject = (name: string) => {
        updateContext({
            flowData: {
                projects: coreContext.flowData.projects?.filter((project: Project) => project.name !== name),
            },
        })
    }
    ...
    return (
        <StepLayout aside={<Aside />}>
            ...
            <p className="text-muted mt-2">Start by adding your projects to get organized and focused.</p>
            ...
            <div className="mt-12 space-y-6">
                ...
                <div className="mt-24">
                    ...
                    <ul className="mt-4 space-x-2 flex items-center">
                        {coreContext.flowData.projects && coreContext.flowData.projects.length > 0 ? (
                            <>
                                {coreContext.flowData.projects.map((project: Project, index: number) => (
                                    <li
                                        key={index}
                                        className="pl-4 pr-0.5 bg-zinc-900 rounded-lg flex items-center justify-between gap-x-4"
                                    >
                                        <p className="text-sm ">{project.name}</p>
                                        <button className="btn-ghost" onClick={() => removeProject(project.name)}>
