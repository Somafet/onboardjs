...
const StepRenderer: React.FC<StepRendererProps> = ({
    ...
    const [isCurrentActiveStepValid, setIsCurrentActiveStepValid] = useState<boolean>(true)
    ...
    const handleStepDataChange = useCallback((data: unknown, isValid: boolean) => {
        const prevData =
            typeof currentActiveStepData === 'object' && currentActiveStepData !== null ? currentActiveStepData : {}
    ...
    const nextButtonLabel = currentStep.payload?.ctaLabel || (state?.isLastStep ? 'Finish' : 'Next')
    ...
    return (
        ...
        >
            ...
            {currentStep.payload?.description && <p>{currentStep.payload?.description}</p>}

            {isLoading && currentStep && <div style={{ minHeight: '50px' }}>{LoadingComponent}</div>}
            ...
            >
                <div>
                    {state?.isSkippable && (
                        <button onClick={() => skip()} disabled={isLoading} style={{ marginRight: '10px' }}>
          ...
          */}
                    {state?.isLastStep && !state?.canGoNext && currentStep.nextStep === null && (
                        <button
