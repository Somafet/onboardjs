...
import { HeadlessOnboardingFlow, HeadlessFlowRenderProps } from './HeadlessOnboardingFlow'
...
describe('HeadlessOnboardingFlow', () => {
    ...
    describe('Basic Functionality', () => {
        ...
        it('should provide all expected props to render function', async () => {
            ...
            render(<HeadlessOnboardingFlow {...defaultProps}>{renderFn}</HeadlessOnboardingFlow>)
        ...
        it('should provide current step information', async () => {
            ...
            await waitFor(() => {
                ...
                expect(screen.getByTestId('step-title')).toHaveTextContent('Welcome Step')
            })
        })
    })
    ...
    describe('Context Management', () => {
        it('should provide current context state', async () => {
            ...
            render(
                <HeadlessOnboardingFlow {...defaultProps} initialContext={initialContext}>
                    {({ state }) => (
                        <div>
                            <span data-testid="context-name">{state?.context?.user?.name}</span>
        ...
        it('should allow updating context', async () => {
            render(
                <HeadlessOnboardingFlow {...defaultProps}>
                    {({ state, updateContext }) => (
                        <div>
                            <span data-testid="context-name">{state?.context?.user?.name}</span>
                            <button onClick={() => updateContext({ user: { name: 'Updated User' } })}>
                                Update Context
                            </button>
                        </div>
                    )}
            ...
            await waitFor(() => {
                expect(screen.getByTestId('context-name')).toHaveTextContent('Updated User')
            })
        })
    })
    ...
    describe('renderStepContent Helper', () => {
        it('should render step content using the helper function', async () => {
            render(
                <HeadlessOnboardingFlow {...defaultProps}>
                    {({ renderStepContent }) => <div data-testid="step-content">{renderStepContent()}</div>}
                </HeadlessOnboardingFlow>
            )
            await waitFor(() => {
        ...
        it('should render custom component when step type is CUSTOM_COMPONENT', async () => {
            ...
            render(
                <HeadlessOnboardingFlow {...defaultProps} steps={customSteps} initialStepId="custom-step">
                    {({ renderStepContent }) => <div data-testid="step-content">{renderStepContent()}</div>}
                </HeadlessOnboardingFlow>
            )
        ...
        it('should show error when component is not registered', async () => {
            ...
            render(
                <HeadlessOnboardingFlow {...defaultProps} steps={invalidSteps} initialStepId="invalid-step">
                    {({ renderStepContent }) => <div data-testid="step-content">{renderStepContent()}</div>}
                </HeadlessOnboardingFlow>
            )
            ...
            await waitFor(() => {
                ...
                expect(consoleSpy).toHaveBeenCalledWith(
                    expect.stringContaining('No component registered for step type/key: NONEXISTENT_COMPONENT')
                )
            })
        ...
        it('should return null when no current step', async () => {
            render(
                <HeadlessOnboardingFlow {...defaultProps} initialStepId={undefined} steps={[]}>
                    {({ renderStepContent }) => (
                        <div data-testid="step-content">{renderStepContent() || 'No content'}</div>
                    )}
                </HeadlessOnboardingFlow>
            )
                    ...
                    <HeadlessOnboardingFlow {...defaultProps}>
                        {({ isLoading, next }) => (
                            <div>
                                ...
                                <button onClick={() => next({ someData: 'test' })}>Next with Data</button>
                            </div>
                        )}
                    </HeadlessOnboardingFlow>
                )
    ...
    describe('Persistence Integration', () => {
        it('should support localStorage persistence', async () => {
            ...
            const TestComponent = () => {
                return (
                    <HeadlessOnboardingFlow {...defaultProps} localStoragePersistence={persistenceConfig}>
                        {({ currentStep }) => (
                            <div>
                                <span data-testid="current-step">{currentStep?.id}</span>
                            </div>
        ...
        it('should support custom data load/persist handlers', async () => {
            ...
            render(
                ...
                >
                    {({ currentStep }) => <span data-testid="current-step">{currentStep?.id}</span>}
                </HeadlessOnboardingFlow>
            )
    ...
    describe('Error Handling', async () => {
        ...
        it('should handle invalid step navigation gracefully', async () => {
            render(
                <HeadlessOnboardingFlow {...defaultProps}>
                    {({ currentStep, goToStep }) => (
                        <div>
                            <span data-testid="current-step">{currentStep?.id}</span>
                            <button onClick={() => goToStep('nonexistent-step')}>Go to Invalid Step</button>
                        </div>
                    )}
                </HeadlessOnboardingFlow>
            )
            ...
            await waitFor(() => {
                expect(screen.getByTestId('current-step')).toHaveTextContent(currentStepText!)
            })
        })
    })
                ...
                <HeadlessOnboardingFlow {...defaultProps} initialStepId={undefined} steps={[]}>
                    {({ currentStep, state }) => (
                        <div>
                            ...
                            <span data-testid="state-exists">{state ? 'exists' : 'null'}</span>
                        </div>
                    )}
                </HeadlessOnboardingFlow>
            )
                    ...
                    {({ currentStep, isLoading }) => {
                        ...
                        return (
                            <div>
                                ...
                                <span data-testid="component-key">{payload?.componentKey ?? ''}</span>
                            </div>
                        )
                    }}
                </HeadlessOnboardingFlow>
