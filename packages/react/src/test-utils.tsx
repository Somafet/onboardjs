...
export const createMockStepComponent = (name: string) => {
    const Component = ({ onDataChange }: StepComponentProps) => (
        <div data-testid={`${name.toLowerCase()}-step`}>
            ...
            {onDataChange && <button onClick={() => onDataChange({}, true)}>{name} Action</button>}
        </div>
    )
...
export const mockStepComponents: StepComponentRegistry = {
    ...
    SINGLE_CHOICE: ({ payload, onDataChange }: StepComponentProps<SingleChoiceStepPayload>) => {
    ...
    MULTIPLE_CHOICE: ({ payload, onDataChange }: StepComponentProps<MultipleChoiceStepPayload>) => {
        const { question, options } = payload
    ...
    CHECKLIST: ({ payload, onDataChange }: StepComponentProps<ChecklistStepPayload>) => {
        const { items } = payload
        return (
    ...
    CONFIRMATION: ({ payload, onDataChange }: StepComponentProps<ConfirmationStepPayload>) => (
    ...
    CUSTOM_COMPONENT: ({ payload }: StepComponentProps<CustomComponentStepPayload>) => (
...
interface CustomRenderOptions extends Omit<RenderOptions, 'wrapper'> {
    onboardingConfig?: Partial<OnboardingEngineConfig & { componentRegistry: StepComponentRegistry }>
    localStoragePersistence?: any
}

export function renderWithOnboardingProvider(ui: React.ReactElement, options: CustomRenderOptions = {}): RenderResult {
    ...
    function Wrapper({ children }: { children: React.ReactNode }) {
        return (
            <OnboardingProvider {...defaultConfig} localStoragePersistence={localStoragePersistence}>
                {children}
            </OnboardingProvider>
        )
    }
...
export function renderWithTestUtils(ui: React.ReactElement, options: RenderOptions = {}): RenderResult {
