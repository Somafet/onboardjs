import { BasePlugin, OnboardingContext, PluginConfig, LoadedData, OnboardingEngine } from '@onboardjs/core'
...
export class SupabasePersistencePlugin<TContext extends OnboardingContext<User>> extends BasePlugin<TContext> {
    ...
    async install(engine: OnboardingEngine<TContext>) {
        ...
        const getUserIdFromContext = (): string | undefined => {
            ...
            const result = key.split('.').reduce((obj, keyPart) => (obj ? obj[keyPart] : undefined), context)
        ...
        engine.setDataLoadHandler(async (): Promise<LoadedData<TContext> | null> => {
    ...
    private _handleError(error: PostgrestError, operation: SupabaseOperation) {
        ...
        if (this.engine) {
            const wrappedError = new Error(`[Supabase Plugin] Operation '${operation}' failed: ${error.message}`)
            ;(wrappedError as any).cause = error

            this.engine.reportError(wrappedError, `SupabasePlugin.${operation}`)
        } else {
            console.error(`[Supabase Plugin] Operation '${operation}' failed:`, error)
        }
    }
}
