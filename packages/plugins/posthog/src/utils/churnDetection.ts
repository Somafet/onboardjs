...
export class ChurnDetectionManager<TContext extends OnboardingContext> {
    ...
    calculateChurnRisk(step: OnboardingStep<TContext>, context: TContext): number {
        ...
        return (
            timeRisk * timeWeight + navigationRisk * navigationWeight + errorRisk * errorWeight + idleRisk * idleWeight
        )
    }
    ...
    private calculateChurnRiskFactors(step: OnboardingStep<TContext>, context: TContext): ChurnRiskFactors {
        const stepKey = step.id.toString()
