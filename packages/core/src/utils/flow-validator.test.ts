...
describe('validateFlow', () => {
    ...
    const expectIssueWithMessage = (
        ...
        expect(issues.some((issue) => issue.message.includes(messagePart) && issue.level === level)).toBe(true)
    }

    const expectNoIssueWithMessage = (issues: ValidationIssue[], messagePart: string) => {
        expect(issues.some((issue) => issue.message.includes(messagePart))).toBe(false)
    }
    ...
    it('should return a warning for an empty flow', () => {
        ...
        expectIssueWithMessage(issues, 'onboarding flow has no steps defined', 'warning')
    })
    ...
    it('should correctly identify multiple issues in a complex broken flow', () => {
        ...
        expectIssueWithMessage(issues, "pointing to a non-existent step ID: 's_non_existent'", 'warning')
        expectIssueWithMessage(issues, "Step 's2' is of type 'CUSTOM_COMPONENT' but is missing 'payload.componentKey'")
        expectIssueWithMessage(issues, "Duplicate step ID found: 's1'")
        expectIssueWithMessage(issues, "Step 's3' is missing a 'type'")
        expectIssueWithMessage(issues, "pointing to a non-existent step ID: 's_skip_non_existent'", 'warning')
    })
    ...
    it('should return an error for SINGLE_CHOICE with no options', () => {
        ...
        const issues = validateFlow(steps)
        expect(issues.length).toBe(1)
        expectIssueWithMessage(issues, "Step 'singleChoice1' is of type 'SINGLE_CHOICE' but has no valid 'options'.")
    })
