...
describe("PerformanceUtils", () => {
  ...
  const runAsyncMeasuredOp = async (
  ...
  describe("findStepById (Caching)", () => {
    it("should return undefined if stepId is null or undefined", () => {
      ...
      expect(
        PerformanceUtils.findStepById(mockSteps, undefined),
      ).toBeUndefined();
    });
    ...
    it("should implement LRU eviction for stepCache", () => {
      ...
      expect((PerformanceUtils as any).stepCache.size).toBe(maxCacheSize);
      expect(
        (PerformanceUtils as any).stepCache.has(`${manySteps.length}-step0`),
      ).toBe(true);
      expect(
        (PerformanceUtils as any).stepCache.has(`${manySteps.length}-step1`),
      ).toBe(false); // step1 should be evicted
    });
  ...
  describe("memoizeStepEvaluation (Caching)", () => {
    ...
    it("should call evaluator if stepId is null and not cache", () => {
      const result = PerformanceUtils.memoizeStepEvaluation(
        null,
        mockContext1,
        mockEvaluator,
      );
    ...
    it("should call evaluator for a new stepId/context combination and cache the result", () => {
      PerformanceUtils.memoizeStepEvaluation(
        "eval1",
        mockContext1,
        mockEvaluator,
      );
    ...
    it("should implement LRU eviction for evaluationCache", () => {
      ...
      expect(
        (PerformanceUtils as any).evaluationCache.has(`eval0-${contextHash0}`),
      ).toBe(false);
    });
  });
  ...
  describe("measurePerformance & measureAsyncPerformance", () => {
    ...
    it("should log a warning for slow sync operations with controlled sequence", () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        ...
        expect.stringContaining(
          "Slow operation detected: slowSync took 150.00ms",
        ),
      );
    ...
    it("measureAsyncPerformance should execute async operation and record metrics", async () => {
      ...
      const result = await PerformanceUtils.measureAsyncPerformance(
        "asyncOp",
        asyncOperation,
      );
    ...
    it("should log a warning for slow async operations with controlled sequence", async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining("PerformanceUtils [WARN]"),
        expect.stringContaining(
          "Slow async operation detected: slowAsync took 250.00ms",
        ),
      );
    ...
    it("should limit performanceMetrics to last 100 measurements", () => {
      ...
      const metrics = (PerformanceUtils as any).performanceMetrics.get(
        "manyMetrics",
      );
    ...
    it("measurePerformance should keep only the last 100 measurements, shifting out oldest", () => {
      ...
      for (let i = 1; i <= totalMeasurements; i++) {
        ...
        const metricsSoFar = (PerformanceUtils as any).performanceMetrics.get(
          operationName,
        ) as number[] | undefined;
        console.log(
          `TEST LOOP: Metrics after duration ${i}: ${JSON.stringify(metricsSoFar?.slice(-5))}`,
        ); // Log last 5
      }
  ...
  describe("getPerformanceStats", () => {
    ...
    it("should calculate stats correctly", () => {
      (PerformanceUtils as any).performanceMetrics.set(
        "testOp",
        [10, 20, 30, 20, 20],
      );
  ...
  describe("hashContext (Indirect Testing)", () => {
    it("should produce different hashes for different flowData values", () => {
      ...
      PerformanceUtils.memoizeStepEvaluation(
        "hashTest",
        { flowData: { keyA: "value1" } },
        evaluator,
      );
      PerformanceUtils.memoizeStepEvaluation(
        "hashTest",
        { flowData: { keyA: "value2" } },
        evaluator,
      );
