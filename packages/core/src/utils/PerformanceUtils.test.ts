...
describe("PerformanceUtils", () => {
  ...
  const runMeasuredOp = (operationName: string, intendedDuration: number) => {
    ...
    console.log(`TEST HELPER: op=${operationName}, i=${intendedDuration}, configuring perf.now to return ${currentCallStartTime} then ${currentCallEndTime}`);
    mockPerfNow
      .mockReturnValueOnce(currentCallStartTime)
      .mockReturnValueOnce(currentCallEndTime);
  ...
  const runAsyncMeasuredOp = async (operationName: string, intendedDuration: number) => {
    ...
    console.log(`TEST HELPER ASYNC: op=${operationName}, i=${intendedDuration}, configuring perf.now to return ${currentCallStartTime} then ${currentCallEndTime}`);
    mockPerfNow
      .mockReturnValueOnce(currentCallStartTime)
      .mockReturnValueOnce(currentCallEndTime);
  ...
  describe("findStepById (Caching)", () => {
    ...
    it("should implement LRU eviction for stepCache", () => {
      ...
      for (let i = 0; i < maxCacheSize + 5; i++) {
        manySteps.push({ id: `step${i}`, type: "INFORMATION", payload: {mainText: "Hello world!"} });
      }
  ...
  describe("memoizeStepEvaluation (Caching)", () => {
    ...
    beforeEach(() => {
      mockEvaluator = vi.fn((id, ctx) => ({ result: `evaluated ${id} with ${JSON.stringify(ctx.flowData)}` }));
    });
    ...
    it("should implement LRU eviction for evaluationCache", () => {
      ...
      const contextHash0 = (PerformanceUtils as any).hashContext({ flowData: { key0: 0 } });
  ...
  describe("debounce", () => {
    it("should call the function only once after the wait time for multiple rapid calls", () => {
      ...
      debouncedFunc(); debouncedFunc(); debouncedFunc();
    ...
    it("should call the function with the latest arguments", () => {
      const func = vi.fn();
      const debouncedFunc = PerformanceUtils.debounce(func, 100);
      debouncedFunc(1); debouncedFunc(2); debouncedFunc(3);
      vi.advanceTimersByTime(100);
  ...
  describe("measurePerformance & measureAsyncPerformance", () => {
    ...
    it("should log a warning for slow sync operations with controlled sequence", () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining("[PerformanceUtils] Slow operation detected: slowSync took 150.00ms")
      );
    ...
    it("should log a warning for slow async operations with controlled sequence", async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining("[PerformanceUtils] Slow async operation detected: slowAsync took 250.00ms")
      );
    ...
    it("should limit performanceMetrics to last 100 measurements", () => {
        ...
        let time = 0;
        tempPerfSpy.mockImplementation(() => { time += 10; return time; });
    ...
    it("measurePerformance should keep only the last 100 measurements, shifting out oldest", () => {
      ...
      for (let i = 1; i <= totalMeasurements; i++) {
        ...
        const metricsSoFar = (PerformanceUtils as any).performanceMetrics.get(operationName) as number[] | undefined;
