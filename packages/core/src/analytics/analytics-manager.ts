...
import { AnalyticsEvent, AnalyticsProvider, AnalyticsConfig, AnalyticsEventPayload } from './types'
...
export class AnalyticsManager<TContext extends OnboardingContext = OnboardingContext> {
    ...
    constructor(config: AnalyticsConfig = {}, logger?: Logger) {
        ...
        this.sessionId = config.sessionId || `session_${Math.random().toString(36).slice(2)}`
        this.logger = logger || new Logger({ debugMode: config.debug, prefix: 'AnalyticsManager' })
    ...
    trackEvent(eventType: string, properties: Record<string, any> = {}): void {
        ...
        if (
            ...
            this.logger.debug(`[AnalyticsManager] Event "${eventType}" skipped due to sampling.`)
            return
        }
        ...
        for (const provider of this.providers) {
            try {
                ...
                this.logger.error(`[AnalyticsManager] Error in analytics provider "${provider.name}":`, error)
            }
        }
    }
    ...
    trackNavigationBack(fromStep: OnboardingStep<TContext>, toStep: OnboardingStep<TContext>, context: TContext): void {
        this.trackEvent('navigation_back', {
            ...
            navigationDistance: this.getStepIndex(fromStep, context) - this.getStepIndex(toStep, context),
        })
    }
    ...
    trackNavigationForward(
        ...
        this.trackEvent('navigation_forward', {
            ...
            fromStepIndex: this.getStepIndex(fromStep, context),
            toStepIndex: this.getStepIndex(toStep, context),
            navigationDistance: this.getStepIndex(toStep, context) - this.getStepIndex(fromStep, context),
        })
    }

    trackNavigationJump(fromStep: OnboardingStep<TContext>, toStep: OnboardingStep<TContext>, context: TContext): void {
        const distance = Math.abs(this.getStepIndex(toStep, context) - this.getStepIndex(fromStep, context))
        this.trackEvent('navigation_jump', {
            ...
            isForwardJump: this.getStepIndex(toStep, context) > this.getStepIndex(fromStep, context),
        })
    }
    ...
    trackStepRenderTime(step: OnboardingStep<TContext>, context: TContext, renderTime: number): void {
        ...
        this.trackEvent('step_render_time', {
            ...
            renderTime,
            stepIndex: this.getStepIndex(step, context),
            isSlowRender: renderTime > (this.config.performanceThresholds?.slowRenderMs || 2000),
        })
    }
    ...
    private enrichEventWithPerformanceData(properties: AnalyticsEventPayload): void {
        if (this.config.enablePerformanceTracking) {
            properties.performanceMetrics = {
                memoryUsage: this.getMemoryUsage(),
                connectionType: this.getConnectionType(),
                renderTimeHistory: Array.from(this.performanceMetrics.stepRenderTimes.values()),
            }
        }
    }

    private getStepIndex(step: OnboardingStep<TContext>, context: TContext): number {
    ...
    private isFirstStep(step: OnboardingStep<TContext>, context: TContext): boolean {
        return this.getStepIndex(step, context) === 0
    }

    private isLastStep(step: OnboardingStep<TContext>, context: TContext): boolean {
    ...
    private calculateChurnRisk(step: OnboardingStep<TContext>, context: TContext, timeOnStep: number): number {
