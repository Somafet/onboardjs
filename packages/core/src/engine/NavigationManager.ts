...
export class NavigationManager<TContext extends OnboardingContext> {
  ...
  async navigateToStep(
    ...
    if (newCurrentStep) {
      ...
      context.flowData._internal!.stepStartTimes![String(newCurrentStep.id)] =
        startTime;
      this.logger.debug(
        `[NavigationManager] Recorded step start time for '${newCurrentStep.id}': ${startTime}`,
      );
      ...
      const flowDuration =
        flowStartedAt && flowStartedAt > 0 ? Date.now() - flowStartedAt : 0;
  ...
  async skip(
    ...
    if (evaluatedSkipTarget === undefined) {
      ...
      if (currentIndex !== -1 && currentIndex < this.steps.length - 1) {
        ...
        let nextInArrayCandidate: OnboardingStep<TContext> | undefined =
          undefined;
  ...
  async goToStep(
    ...
    if (this.stateManager.isLoading) {
      this.logger.debug(
        `[NavigationManager] goToStep(): Ignoring - engine is loading.`,
      );
      return currentStep;
    }
    ...
    if (stepSpecificData) {
      ...
      this.logger.debug(
        `[NavigationManager] goToStep(): Context flowData updated with step-specific data.`,
      );
    }
