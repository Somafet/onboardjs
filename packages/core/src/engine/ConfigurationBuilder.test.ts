...
describe('ConfigurationBuilder', () => {
    ...
    describe('mergeConfigs', () => {
        ...
        it('should deeply merge flowData within initialContext', () => {
            ...
            expect(merged.initialContext?.flowData?.mergedFlowKey).toBe('updateValue') // From update
        ...
        it('should concatenate plugins arrays', () => {
            ...
            expect(merged.plugins?.map((p) => p.name)).toEqual(['pluginA', 'pluginB'])
        })
        ...
        it('should handle empty or undefined parts in current or updates gracefully', () => {
            ...
            let merged = ConfigurationBuilder.mergeConfigs(emptyBase, updatesWithData)
        ...
        it('should handle current.initialContext being undefined', () => {
            ...
            const merged = ConfigurationBuilder.mergeConfigs(currentNoCtx, updatesWithCtx)
        ...
        it('should handle updates.initialContext being undefined', () => {
            ...
            const merged = ConfigurationBuilder.mergeConfigs(currentWithCtx, updatesNoCtx)
            expect(merged.initialContext?.user?.id).toBe('u1')
            expect(merged.initialContext?.flowData?.key).toBe('v')
        })
        ...
        it('should handle current.plugins being undefined', () => {
            ...
            const merged = ConfigurationBuilder.mergeConfigs(currentNoPlugins, updatesWithPlugins)
            expect(merged.plugins).toHaveLength(1)
    ...
    describe('validateConfig', () => {
        ...
        it('should detect steps without an ID', () => {
            const config: OnboardingEngineConfig<TestContext> = {
                steps: [mockStep1, { type: 'INFORMATION', payload: {} } as OnboardingStep<TestContext>],
            }
            const result = ConfigurationBuilder.validateConfig(config)
            expect(result.isValid).toBe(false)
        ...
        it('should validate checklist steps: missing items array', () => {
            const config: OnboardingEngineConfig<TestContext> = {
                steps: [{ id: 'c1', type: 'CHECKLIST', payload: { dataKey: 'd' } } as any],
            }
            const result = ConfigurationBuilder.validateConfig(config)
            expect(result.isValid).toBe(false)
        ...
        it('should detect if initialStepId does not exist in steps', () => {
            ...
            const result = ConfigurationBuilder.validateConfig(config)
            expect(result.isValid).toBe(false)
            expect(result.errors).toContain('Initial step ID nonExistent not found in steps')
        })
    ...
    describe('createDefaultConfig', () => {
        it('should return a config with empty steps, empty flowData in initialContext, and empty plugins', () => {
            const defaultConfig = ConfigurationBuilder.createDefaultConfig<TestContext>()
    ...
    describe('cloneConfig', () => {
        it('should create a deep clone of the configuration object', () => {
            ...
            const originalConfigWithoutFunctions = JSON.parse(JSON.stringify(originalConfig))
            ...
            expect(clonedConfig.initialContext).not.toBe(originalConfig.initialContext)
            expect(clonedConfig.initialContext?.flowData).not.toBe(originalConfig.initialContext?.flowData)
            expect(clonedConfig.initialContext?.user).not.toBe(originalConfig.initialContext?.user)
