...
type NotifyListenersSequentialFn = <
  E extends keyof EventListenerMap<TestContext>,
>(
  eventType: E,
  ...args: Parameters<EventListenerMap<TestContext>[E]>
) => Promise<void>;
...
describe("EventHandlerRegistry", () => {
  ...
  beforeEach(() => {
    ...
    registry = new EventHandlerRegistry<TestContext>(
      mockEventManager as any as EventManager<TestContext>,
    );
  });
    ...
    describe(`${String(methodName)} (Convenience)`, () => {
      it(`should call eventManager.addEventListener with '${eventType}' and the listener`, () => {
        ...
        switch (methodName) {
          ...
          default:
            ...
            throw new Error(
              `Test setup error: Unhandled method name ${String(methodName)}`,
            );
        }
      ...
      it(`should return the unsubscribe function`, () => {
        ...
        switch (methodName) {
          ...
          default:
            throw new Error(
              `Test setup error: Unhandled method name ${String(methodName)} for unsubscribe`,
            );
        }
        expect(unsubscribe).toBe(sharedMockUnsubscribeFn);
      });
  ...
  describe("Plugin Compatibility Methods", () => {
    describe("addBeforeStepChangeListener", () => {
      ...
      it("wrapped listener should call findStepById with targetStepId", async () => {
        const originalListener = vi.fn();
        registry.addBeforeStepChangeListener(originalListener);
        const wrappedListener = (mockEventManager.addEventListener as Mock).mock
          .calls[0][1];
  ...
  describe("Utility Methods", () => {
    ...
    describe("getListenerCount", () => {
      it("should call eventManager.getListenerCount and return its result", () => {
        ...
        expect(mockEventManager.getListenerCount).toHaveBeenCalledWith(
          "stateChange",
        );
        expect(count).toBe(5);
      });
