...
import { EventListenerMap, UnsubscribeFunction, BeforeStepChangeListener, BeforeStepChangeEvent } from "./types";
...
describe("EventHandlerRegistry", () => {
  ...
  convenienceMethodsConfig.forEach(({ eventType, methodName }) => {
    describe(`${String(methodName)} (Convenience)`, () => {
      it(`should call eventManager.addEventListener with '${eventType}' and the listener`, () => {
        ...
        switch (methodName) {
          ...
          case "onBeforeStepChange":
            ...
            registry.onBeforeStepChange(listener as BeforeStepChangeListener<TestContext>);
            break;
          default:
      ...
      it(`should return the unsubscribe function`, () => {
        ...
        switch (methodName) {
          case "onStepChange": unsubscribe = registry.onStepChange(listener); break;
          case "onFlowComplete": unsubscribe = registry.onFlowComplete(listener); break;
          case "onStepActive": unsubscribe = registry.onStepActive(listener); break;
          case "onStepComplete": unsubscribe = registry.onStepComplete(listener); break;
          case "onContextUpdate": unsubscribe = registry.onContextUpdate(listener); break;
          case "onError": unsubscribe = registry.onError(listener); break;
          case "onStateChange": unsubscribe = registry.onStateChange(listener); break;
          case "onBeforeStepChange": unsubscribe = registry.onBeforeStepChange(listener as BeforeStepChangeListener<TestContext>); break;
          default:
  ...
  describe("Plugin Compatibility Methods", () => {
      describe("addBeforeStepChangeListener", () => {
        ...
        const mockCurrentStep = { id: "current", type: "INFO" } as unknown as OnboardingStep<TestContext>;
        const mockNextStep = { id: "next", type: "INFO" } as unknown as OnboardingStep<TestContext>;
