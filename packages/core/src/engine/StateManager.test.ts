...
describe('StateManager', () => {
    ...
    const createMockFlowContext = (overrides: Partial<FlowContext> = {}): FlowContext => ({
        flowId: 'test-flow-id',
        flowName: 'Test Flow',
        flowVersion: '1.0.0',
    ...
    beforeEach(() => {
        ...
        mockEventManager = new EventManager() as Mocked<EventManager<OnboardingContext>>
        ...
        stateManager = new StateManager(mockEventManager, mockSteps, 'step1', createMockFlowContext())
    })
    ...
    describe('getState', () => {
        ...
        it('should identify an explicit last step where nextStep is null', () => {
            const state = stateManager.getState(mockSteps[4], mockContext, ['step1', 'step2']) // step3
        ...
        it('should find previous step from `previousStep` property', () => {
            ...
            const manager = new StateManager(mockEventManager, steps, 'A', createMockFlowContext())
    ...
    describe('setState', () => {
        ...
        it('should not notify or call onContextChange if state does not change', () => {
            ...
            stateManager.setState(() => ({ isLoading: false }), null, mockContext, [], onContextChange)
    ...
    describe('notifyStateChange', () => {
        it('should call eventManager.notifyListeners with the latest state', () => {
            ...
            expect(mockEventManager.notifyListeners).toHaveBeenCalledWith('stateChange', {
                state: expect.any(Object),
            })

            // Check that the state passed to the listener is correct
            const emittedState = mockEventManager.notifyListeners.mock.calls[0][1] as {
                state: EngineState<OnboardingContext>
            }
    ...
    describe('Progress Calculation in getState', () => {
        ...
        it('should respect conditions when calculating total steps', () => {
            ...
            const state = stateManager.getState(mockSteps[0], contextWithConditionFalse, [])
        ...
        it('should respect conditions when calculating total and current step number', () => {
            ...
            const state = stateManager.getState(mockSteps[2], contextWithConditionFalse, [])
            expect(state.totalSteps).toBe(4)
            expect(state.completedSteps).toBe(0)
            expect(state.progressPercentage).toBe(0)
        ...
        it('should not count a completed step if its condition is now false', () => {
            ...
            const state = stateManager.getState(mockSteps[2], contextWithChangedCondition, [])
        ...
        it('should handle the case with 0 total relevant steps', () => {
            ...
            const conditionalOnlySteps: OnboardingStep<OnboardingContext>[] = [mockSteps[1], mockSteps[2]]
            ...
            const state = specificManager.getState(null, contextWithNoRelevantSteps, [])
        ...
        it('should handle a context with no _internal data gracefully', () => {
            ...
            const state = stateManager.getState(mockSteps[0], contextWithoutInternal, [])
            expect(state.completedSteps).toBe(0)
            expect(state.progressPercentage).toBe(0)
        })
    })
    ...
    describe('getRelevantSteps', () => {
        it('should return all steps when no conditions are present', () => {
            ...
            const manager = new StateManager(mockEventManager, stepsWithoutConditions, 'step1', createMockFlowContext())

            const relevantSteps = manager.getRelevantSteps(mockContext)

            expect(relevantSteps).toHaveLength(3)
            expect(relevantSteps.map((s) => s.id)).toEqual(['step1', 'step2', 'step3'])
        })
        ...
        it('should filter out steps with failing conditions', () => {
            ...
            const relevantSteps = stateManager.getRelevantSteps(contextWithFalseCondition)
        ...
        it('should include steps with passing conditions', () => {
            ...
            const relevantSteps = stateManager.getRelevantSteps(contextWithTrueConditions)

            // Should include all steps
            expect(relevantSteps).toHaveLength(5)
            const stepIds = relevantSteps.map((step) => step.id)
            expect(stepIds).toEqual(['step1', 'conditionalStep', 'conditionalStep2', 'step2', 'step3'])
        })
        ...
        it('should handle dynamic conditions that depend on context data', () => {
            ...
            const manager = new StateManager(mockEventManager, dynamicSteps, 'base', createMockFlowContext())
            ...
            expect(adminRelevantSteps.map((s) => s.id)).toEqual(['base', 'userTypeAdmin'])
            expect(userRelevantSteps.map((s) => s.id)).toEqual(['base', 'userTypeUser'])
        })
    })
    ...
    describe('getStepById', () => {
        ...
        it('should return the correct step when found by number id', () => {
            ...
            const manager = new StateManager(mockEventManager, numericSteps, 1, createMockFlowContext())
        ...
        it('should handle mixed id types correctly', () => {
            ...
            const manager = new StateManager(mockEventManager, mixedSteps, 'stringId', createMockFlowContext())
    ...
    describe('getCompletedSteps', () => {
        it('should return empty array when no steps are completed', () => {
            ...
            const completedSteps = stateManager.getCompletedSteps(contextWithoutCompletions)
        ...
        it('should return empty array when _internal data is missing', () => {
            ...
            const completedSteps = stateManager.getCompletedSteps(contextWithoutInternal)
        ...
        it('should return completed steps that exist in the steps array', () => {
            ...
            const completedSteps = stateManager.getCompletedSteps(contextWithCompletions)
        ...
        it("should ignore completed step IDs that don't exist in the steps array", () => {
            ...
            const completedSteps = stateManager.getCompletedSteps(contextWithInvalidCompletions)
        ...
        it('should handle numeric step IDs correctly', () => {
            ...
            const completedSteps = manager.getCompletedSteps(contextWithNumericCompletions)
        ...
        it('should return steps in the same order as they appear in the original steps array', () => {
            ...
            const completedSteps = stateManager.getCompletedSteps(contextWithAllCompletions)

            expect(completedSteps).toHaveLength(5)
            const completedIds = completedSteps.map((step) => step.id)
            // Should maintain the original order from mockSteps array
            expect(completedIds).toEqual(['step1', 'conditionalStep', 'conditionalStep2', 'step2', 'step3'])
        })
        ...
        it('should include completed conditional steps regardless of their current condition', () => {
            ...
            const completedSteps = stateManager.getCompletedSteps(contextWithCompletedConditionalStep)
