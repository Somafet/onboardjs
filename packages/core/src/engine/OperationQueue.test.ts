...
describe("OperationQueue", () => {
  ...
  describe("Priority Handling", () => {
    ...
    it("should handle enqueueUrgent correctly", async () => {
      ...
      const promises = [
        queue.enqueue(normalOp, 1),
        queue.enqueueUrgent(urgentOp),
      ];
  ...
  describe("Error Handling", () => {
    ...
    it("should handle operation errors and reject the promise", async () => {
      ...
      await expect(queue.enqueue(failingOperation)).rejects.toThrow(
        "Operation failed",
      );
  ...
  describe("Statistics and Monitoring", () => {
    ...
    it("should provide accurate queue statistics", async () => {
      ...
      expect(stats.oldestOperationAge).toBeLessThanOrEqual(
        Date.now() - enqueueTime2 + 5,
      ); // Check age of op2
      ...
      expect(stats.oldestOperationAge).toBeGreaterThanOrEqual(
        Date.now() - enqueueTime2 - 10,
      );
