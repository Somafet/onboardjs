...
const mockSteps: OnboardingStep[] = [
    ...
    },
    {
        ...
        nextStep: (context) => (context.data['choose-path'] === 'dev' ? 'dev-tools' : 'design-tools'),
    },
    {
        id: 'dev-tools',
        type: 'CHECKLIST',
...
describe('StepJSONParser', () => {
    describe('Core Serialization (toJSON)', () => {
        ...
        it("should handle functions with 'serialize' option", () => {
            ...
            const parsed = JSON.parse(result.data!)
            const choosePathStep = parsed.steps.find((s: any) => s.id === 'choose-path')
            expect(choosePathStep.nextStep.__isFunction).toBe(true)
            expect(choosePathStep.nextStep.__functionBody).toContain('context.data["choose-path"]')
        })
        ...
        it('should use a custom function serializer', () => {
            const customSerializer = vi.fn((fn, propName, stepId) => `CUSTOMIZED::${stepId}::${propName}`)
            const result = StepJSONParser.toJSON(mockSteps, {
            ...
            expect(choosePathStep.nextStep.__functionBody).toBe('CUSTOMIZED::choose-path::nextStep')
        })
    })
    ...
    describe('Core Deserialization (fromJSON)', () => {
        ...
        it('should correctly deserialize functions', () => {
            ...
            expect((choosePathStep!.nextStep as Function)(mockContext)).toBe('dev-tools')
        })

        it('should use a custom function deserializer', () => {
            const customDeserializer = vi.fn((fnString, propName, stepId) => () => `DESERIALIZED::${stepId}`)
            const result = StepJSONParser.fromJSON(serializedJSON, {
    ...
    describe('Convenience Methods', () => {
        ...
        it('clone() should create a deep copy of steps', () => {
            ...
            expect(JSON.stringify(clonedSteps)).toEqual(
                JSON.stringify(StepJSONParser.deserialize(StepJSONParser.serialize(mockSteps)!))
            )
        })
    })
    ...
    describe('getExportableData', () => {
        ...
        it('should use a custom filename and pass options down to toJSON', () => {
            ...
            const result = StepJSONParser.getExportableData(mockSteps, customFilename, customOptions)
    ...
    describe('StepJSONParserUtils Namespace', () => {
        ...
        it('getStepTypesFromJSON() should return an array of step types', () => {
            ...
            expect(types).toEqual(['INFORMATION', 'SINGLE_CHOICE', 'CHECKLIST', 'CUSTOM_COMPONENT', 'CONFIRMATION'])
        ...
        it('getStepIdsFromJSON() should return an array of step IDs', () => {
            ...
            expect(ids).toEqual(['welcome', 'choose-path', 'dev-tools', 'custom-comp', 'final'])
        ...
        it('hasCustomComponents() should detect custom component steps', () => {
            ...
            const noCustomCompJson = StepJSONParser.serialize(mockSteps.filter((s) => s.type !== 'CUSTOM_COMPONENT'))!
            expect(StepJSONParserUtils.hasCustomComponents(noCustomCompJson)).toBe(false)
        })
    ...
    describe('Error Handling and Edge Cases', () => {
        ...
        describe('StepJSONParserUtils', () => {
            it('should handle invalid JSON gracefully', () => {
                ...
                expect(StepJSONParserUtils.getStepTypesFromJSON(invalidJson)).toEqual([])
                expect(StepJSONParserUtils.getStepIdsFromJSON(invalidJson)).toEqual([])
                expect(StepJSONParserUtils.hasCustomComponents(invalidJson)).toBe(false)
